...
class NeMoTranscriber:
    ...
    @classmethod
    def transcribe_chunk(cls, audio_chunk, prev_states=None):
        """
        audio_chunk: np.array of shape (chunk_size_samples,); dtype=np.int16
        """
        if prev_states is None:
            prev_states = cls.get_initial_states()
        ...
        processed_signal, processed_signal_length = cls.preprocess_audio(audio_chunk)
...
if __name__ == "__main__":
    ...
    states = None
    for i in range(0, len(audio), int(sr * NeMoTranscriber.chunk_size_ms / 1000)):
        chunk = audio[i : i + int(sr * NeMoTranscriber.chunk_size_ms / 1000)]
        if len(chunk) < int(sr * NeMoTranscriber.chunk_size_ms / 1000):
            chunk = np.pad(chunk, (0, int(sr * NeMoTranscriber.chunk_size_ms / 1000) - len(chunk)))
